function [TRANS] = solve_transSSJ(param,grid,A,SS,J)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Preallocation
    r                   = SS.r*ones(1,param.TT) ; % !!! this is the initial guess for the path of r !!!
...
% iterate on transition path
    for iter=1:param.maxiter
        ...
        % given the guess of interest rate recover equilibrium objects
            ...
            Kdem = ((r+param.delta)./(param.alpha*A*param.labor^(1-param.alpha))).^(1/(param.alpha-1));    % implied guess for capital stock
        ...
        % updatee interest rate guess and do the previous steps again
        if (max(abs(r - r_new)) < param.tol_r)
            break
        else
            
            % update interest rate path based on Newtons method (first order Taylor approx) using the sequence space Jacobian
            rnew = r(1:end-1)' - J\(Ksup(1:end-1)-Kdem(1:end-1))'; 
            rnew = [rnew' SS.r]; % anchor last period, not to be updated
            r = rnew; % this method is so robust that you often do not need slow updating
